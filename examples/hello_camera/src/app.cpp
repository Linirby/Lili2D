...
App::App()
  : lili::Game("hello_camera - Lili2D", 600, 400)
{
    setTps(20.0f);
    ...
    camera_center = lili::Circle(
        renderer,
        lili::CircleShape(lili::Vec2(0.0f, 0.0f), 5.0f, 16),
    ...
    green_rect = lili::Rect(
        renderer,
        lili::RectShape(100.0f, 100.0f, 75.0f, 75.0f),
        lili::Vec4(0.0f, 1.0f, 0.0f, 1.0f)
    );
    blue_rect = lili::Rect(
        renderer,
        lili::RectShape(400.0f, 250.0f, 50.0f, 200.0f),
    ...
    action_map.add(
        "MoveRight", { lili::Key::L, lili::Key::D, lili::Key::RIGHT }
    );
...
App::onUpdate(float dt)
{
    ...
    lili::Vec2 dir{};
    if (action_map.isHeld("MoveUp"))
        dir.y -= 1.0f;
    if (action_map.isHeld("MoveDown"))
        dir.y += 1.0f;
    if (action_map.isHeld("MoveLeft"))
        dir.x -= 1.0f;
    if (action_map.isHeld("MoveRight"))
        dir.x += 1.0f;

    cam_pos = camera.getPosition() + dir * (100.0f * dt);
    camera.setPosition(cam_pos);

    if (action_map.isHeld("Zoom"))
        camera_zoom += 2.0f * dt;
    if (action_map.isHeld("Dezoom"))
        camera_zoom -= 2.0f * dt;

...
App::App()
  : lili::Game("hello_collision - Lili2D", 800, 800)
{
    lili::Renderer* renderer = getRenderer();
    cursor_rect = lili::Rect(
        renderer,
        lili::RectShape(0.0f, 0.0f, 75.0f, 75.0f),
        lili::Vec4(0.0f, 0.0f, 1.0f, 1.0f)
    );
    cursor_circle = lili::Circle(
        renderer,
        lili::CircleShape(lili::Vec2(0.0f, 0.0f), 30.0f, 16),
        lili::Vec4(0.0f, 0.0f, 1.0f, 1.0f)
    );
    ...
    random_rect = lili::Rect(
        renderer,
        lili::RectShape(350.0f, 350.0f, 100.0f, 100.0f),
    ...
    if (event.type() == lili::EventType::KEYBOARD) {
        ...
        if (kb.action == lili::KeyAction::PRESSED) {
            if (kb.key == lili::Key::ESCAPE)
                shutdown();
            else if (kb.key == lili::Key::NUM_1)
                active_shape = ShapeMode::Rect;
            else if (kb.key == lili::Key::NUM_2)
                active_shape = ShapeMode::Circle;
            else if (kb.key == lili::Key::NUM_3)
                active_shape = ShapeMode::Line;
        }
    }
}

...
UIPipeline::UIPipeline(UIPipeline &&other) noexcept
	: device(other.device),
	window(other.window),
	shader(other.shader),
...
UIPipeline& UIPipeline::operator=(UIPipeline &&other) noexcept {
	if (this != &other) {
		if (pipeline) SDL_ReleaseGPUGraphicsPipeline(device, pipeline);
		device = other.device;
		window = other.window;
...
SDL_GPUGraphicsPipeline *UIPipeline::getSdlPipeline() {
	return pipeline;
}

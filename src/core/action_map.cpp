...
ActionMap::addMouseButton(std::string_view action, MouseButton btn)
{
    ...
    if (std::find(btns.begin(), btns.end(), btn) != btns.end())
        return false;
    btns.push_back(btn);
    return true;
}
...
ActionMap::isHeld(std::string_view action) const noexcept
{
    ...
    for (Key key : it->second.keys)
        if (keyboard.held(static_cast<Scancode>(key)))
            return true;
...
ActionMap::isJustPressed(std::string_view action) const noexcept
{
    ...
    for (Key key : it->second.keys)
        if (keyboard.justPressed(static_cast<Scancode>(key)))
            return true;
    for (MouseButton btn : it->second.mouse_buttons)
        if (mouse.justPressed(btn))
            return true;
    return false;
}
...
ActionMap::isJustReleased(std::string_view action) const noexcept
{
    ...
    for (Key key : it->second.keys)
        if (keyboard.justReleased(static_cast<Scancode>(key)))
            return true;
    for (MouseButton btn : it->second.mouse_buttons)
        if (mouse.justReleased(btn))
            return true;
    return false;
}

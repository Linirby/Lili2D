...
ResourceManager<Texture>& AssetManager::textures() {
    return texture_manager;
}

ResourceManager<Shader>& AssetManager::shaders() {
    return shader_manager;
}

ResourceManager<BitmapFont>& AssetManager::fonts() {
    return font_manager;
}

ResourceManager<AtlasMap>& AssetManager::atlases() {
...
Shader* AssetManager::loadShader(
    ...
    return get().shaders().load(
        key,
        vertPath,
        [device, vertPath, fragPath](const std::string&) {
            return std::make_unique<Shader>(device, vertPath, fragPath);
        },
        scope
    );
}
...
BitmapFont* AssetManager::loadFont(
    const std::string& key,
    Renderer* renderer,
    const std::string& path,
    uint8_t cols,
...
BitmapFont* AssetManager::getFont(const std::string& key) {
    return get().fonts().get(key);
}

AtlasMap* AssetManager::loadAtlas(
    const std::string& key,
    Renderer* renderer,
    const std::string& path,
    int cols,
...
AtlasMap* AssetManager::getAtlas(const std::string& key) {
    return get().atlases().get(key);
}

size_t AssetManager::unloadScope(const std::string& scope) {
    size_t count = 0;

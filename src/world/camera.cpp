...
void
Camera::setPosition(const Vec2& pos) {
    position = pos;
    is_dirty = true;
}

void
Camera::setRotation(float degree) {
    rotation = degree;
    is_dirty = true;
}
...
Mat3
Camera::getViewMatrix(float width, float height) const {
...
Mat3
Camera::getProjection(float width, float height) const {
    return Mat3::orthographic(0.0f, width, 0.0f, height);
}

AABB2
Camera::getViewportBounds(float width, float height) const {

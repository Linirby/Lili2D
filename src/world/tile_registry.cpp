...
bool
TileRegistry::hasTile(const std::string& key) const {
    return hasAsset(key);
}

size_t
TileRegistry::tileCount() const {
    return assetCount();
}

uint16_t
TileRegistry::getTileID(const std::string& key) const {
    return getAssetID(key);
}

const Tile&
TileRegistry::getTile(const std::string& key) const {
    return getAsset(key);
}

const Tile&
TileRegistry::getTile(uint16_t tile_id) const {

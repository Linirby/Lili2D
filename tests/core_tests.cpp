...
TEST_CASE("ResourceManager Generic Cache", "[core][resource_manager]") {
    ...
    DummyResource* r1 = manager.load("hero", "assets/hero.png", dummy_loader, "level1");
    REQUIRE(r1 != nullptr);
    ...
    manager.emplace("custom", std::make_unique<DummyResource>(DummyResource{"custom_val"}), "ui");
